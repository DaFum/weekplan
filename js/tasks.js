...
const TASK_NAME_ERROR_MESSAGE =
  'Aufgabenname muss zwischen 1 und 100 Zeichen lang sein.';
...
export function saveTask(event) {
  ...
  const { idInput, nameInput, dateInput, durationInput, errorEl } =
    getTaskFormElements();
  ...
  const newTasks = safeTasks.map((task) => {
    ...
    if (task.id === taskId) {
      ...
      if (newErledigt) {
        ...
        if (typeof sounds?.complete?.triggerAttackRelease === 'function') {
...
export function deleteTask(taskId) {
  ...
  const safeTasks = Array.isArray(tasks) ? tasks : [];
  updateState({
    tasks: safeTasks.filter((task) => Boolean(task) && task?.id !== taskId),
  });
}
...
export function cleanupOldTasks() {
  ...
  updateState({
    tasks: safeTasks.filter((task) => {
      ...
      return typeof isoDate === 'string' && isoDate >= startOfCurrentWeekISO;
    }),
  });
}
...
function getTaskFormElements() {
  ...
  return {
    ...
    durationInput:
      durationInput instanceof HTMLInputElement ? durationInput : null,
...
function updateExistingTask(taskId, taskData) {
  const { tasks } = getState();
  const safeTasks = Array.isArray(tasks) ? tasks : [];
  const taskIndex = safeTasks.findIndex(
    (task) => Boolean(task) && task?.id === taskId
  );
