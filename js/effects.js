...
export function starteKonfetti(
  container = document.getElementById('konfetti-container') || document.body
) {
  ...
  for (let i = 0; i < KONFETTI_COUNT; i++) {
    ...
    konfetti.style.top = `${
      Math.random() * KONFETTI_POSITION_PERCENT_RANGE -
      KONFETTI_TOP_OFFSET_PERCENT
    }%`;
    konfetti.style.backgroundColor = `hsl(${
      Math.random() * KONFETTI_HUE_RANGE
    }, 100%, 50%)`;
    konfetti.style.transform = `scale(${
      Math.random() * KONFETTI_SCALE_RANDOM_FACTOR + KONFETTI_SCALE_MIN
    })`;
    konfetti.style.animationDelay = `${
      Math.random() * KONFETTI_ANIMATION_DELAY_MAX_S
    }s`;
