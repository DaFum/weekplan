...
async function loadTone() {
  try {
    ...
    const timeoutPromise = new Promise((_, reject) => {
      setTimeout(
        () => reject(new Error('Tone.js import timed out')),
        LOAD_TIMEOUT
      );
    });
    const mod = await Promise.race([importPromise, timeoutPromise]);
    const Tone = mod?.default ?? mod?.Tone ?? mod;
    if (Tone?.Synth) return Tone;
  } catch (error) {
    console.warn(
      'Tone.js dynamic import failed, falling back to script tag.',
      error
    );
  }
  ...
  try {
    ...
    console.error(
      'Tone.js could not be loaded or audio synthesizers could not be created. Audio functions are disabled.',
      error
    );
    return null;
  }
